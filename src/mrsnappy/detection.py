...
def _local_size(ndim: int, neighborhood: int) -> tuple[int, ...]:
    size = max(int(neighborhood) * 2 + 1, 1)
    return (size,) * ndim
...
def detect_candidates(volume: np.ndarray, cfg: dict) -> DetectionResult:
    ...
    threshold = float(cfg.get("threshold_value", 0.1))
    neighborhood = int(cfg.get("maxima_neighborhood", 2))
    sigma = float(cfg.get("sigma_value", 1.35))
    max_candidates = cfg.get("max_candidates")
    peak_kwargs: dict[str, int] = {}
    ...
    if method in {"log", "laplacian_of_gaussian", "laplacian of gaussian"}:
        ...
        coords = peak_local_max(
            response,
            min_distance=max(neighborhood, 1),
    ...
    elif method in {"regional", "simple_regional"}:
        response = volume
        coords = peak_local_max(
            response,
            min_distance=max(neighborhood, 1),
        ...
        if coords.size == 0:
            ...
            scores = response[tuple(coords.T)]
    elif method in {"extended", "extended_maxima", "hmax", "h_max", "h-max", "h_maxima", "h-maxima"}:
        response = volume
        ...
        raise ValueError(f"Unsupported maxima method: {cfg.get('maxima_method')}")
